...
const RenderFileSchema = z.object({
  ...
  startFrame: z.number().min(1),
  endFrame: z.number().min(1),
});
...
export function RenderFile() {
  ...
  const form = useForm<z.infer<typeof RenderFileSchema>>({
    ...
    defaultValues: { file: undefined, startFrame: 1, endFrame: 250 },
  })
  ...
  const onSubmit = async (values: z.infer<typeof RenderFileSchema>) => {
    ...
    try {
      ...
      if (!token) { toast.error("Authentication error. Please log in again."); setLoading(false); return }
      ...
      const responseText = await signedUrlApiResponse.text()
  ...
  if (loading) return (
    <motion.div
      className="flex flex-col items-center justify-center gap-4 p-8 rounded-2xl bg-gray-900 text-white shadow-inner"
  ...
  return (
    ...
    >
      <div className="flex flex-col items-center gap-2 text-center">
        ...
        <p className="text-gray-400 text-sm">Drag & drop your <span className="font-semibold text-orange-500">.blend</span> or <span className="font-semibold text-orange-500">.zip</span> file and set the frame range below.</p>
      </div>
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            ...
            <FormItem>
              <FormControl>
                ...
                >
                  ...
                  <p className="font-semibold">{form.watch("file")?.[0]?.name || "Drag & drop file here or click to browse"}</p>
                  <p className="text-xs text-gray-500 mt-1"></p>
                  <Input type="file" accept=".zip,.blend" ref={fileInputRef} className="hidden" onChange={(e) => field.onChange(e.target.files)} />
                </motion.div>
              </FormControl>
              <FormMessage />
            </FormItem>
          ...
          <div className="grid grid-cols-2 gap-4">
            <FormField control={form.control} name="startFrame" render={({ field }) => (
              <FormItem>
                ...
                <FormControl>
                  <Input type="number" placeholder="e.g., 1" {...field} className="bg-gray-800 text-white border-gray-700 focus:border-orange-500 focus:ring-orange-500" />
                </FormControl>
                <FormMessage />
              </FormItem>
            )} />
            <FormField control={form.control} name="endFrame" render={({ field }) => (
              <FormItem>
                ...
                <FormControl>
                  <Input type="number" placeholder="e.g., 250" {...field} className="bg-gray-800 text-white border-gray-700 focus:border-orange-500 focus:ring-orange-500" />
                </FormControl>
                <FormMessage />
              </FormItem>
            )} />
          ...
          <motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
            <Button type="submit" className="w-full bg-orange-500 hover:bg-orange-600 transition shadow-lg shadow-orange-500/30 text-lg py-4 font-bold" disabled={loading || !form.formState.isValid}>
              Render File
            </Button>
          </motion.div>
        </form>
